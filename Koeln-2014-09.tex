...
\texttt{SCOREP\_ENABLE\_PROFILING} & Enable generation of profiles (default=true)\\
\texttt{SCOREP\_ENABLE\_TRACING} & Enable the generation of traces (default=false)\\
\texttt{SCOREP\_TOTAL\_MEMORY} & Total memory in bytes used for Score-P per process (default=16M)\\
...
There is a DBUS problem on TRACE2, so start Vampir with \texttt{-{}-no-dbus}.\\[2ex]

\textbf{Settings for \emph{nomachine 4} client connection}\\
Protocol: SSH, Port 22\\
Advanced: \emph{Use the NoMachine login}\\[2ex]

\textbf{Open small traces directly in Vampir}\\
\texttt{ \% vampir -{}-no-dbus scorep\_run\_trace/traces.otf2}\\[2ex]

\textbf{Open large traces using VampirServer}\\
1. Launch analysis server on remote machine (uses a batch job for performance)\\
\texttt{ \% ssh remote-machine}\\
\texttt{ \% vampirserver start -n 8 }\\
\texttt{ Submitting PBS batch job (this might take a while)... }\\
\texttt{ Batch job is submitted - Job ID: 857357.master }\\
\texttt{ Running 15 analysis processes... (abort with vampirserver stop 17950)}\\
\texttt{ VampirServer <17950> listens on: \textcolor{red}{node123:30085}}\\[2ex]

2. Open Vampir in \emph{nomachine} session and connect to VampirServer (listening on node123:30085)\\
\texttt{ \% vampir -{}-no-dbus \textcolor{red}{node123:30085}:scorep\_run\_trace/traces.otf2}\\
or use \emph{Open Other/Remote File} in Vampir.\\[2ex]

3. Shutdown VampirServer and batch job when finished\\
\texttt{ \% vampirserver stop 17950}\\
...
\textcolor{red}{PAPI counters are not available on the TRACE2 system}\\
\texttt{\% export SCOREP\_METRIC\_PAPI=PAPI\_L2\_DCM:!CPU\_TEMP1}\\
...
\texttt{\% export SCOREP\_METRIC\_RUSAGE=ru\_stime:ru\_majflt}\\
